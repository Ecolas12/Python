...
def building_schedule():
    draft = input("what class to add?", )
    draft = draft.title()
    draft_display = draft.split(",")
...
def adding_classes(new_list):
    schedule_addition = input("Must have 5 class, enter courses to add:", )
...
def main():
    ...
    drop_from_schedule = dropping_classes(add_to_schedule)
